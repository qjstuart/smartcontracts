    ...
    async handleClick(event) {
        ...
        if (isValid) {
            try {
                ...
                await this.props.loanPlatform.methods.guarantee(this.state.index, this.state.gInterest).send({ from: accounts[0], value: this.state.value})
            } catch (error) {
